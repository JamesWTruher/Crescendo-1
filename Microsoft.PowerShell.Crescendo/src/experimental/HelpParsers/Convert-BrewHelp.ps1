...
$exe = "brew"
$helpChar = "--help"
$commandPattern = "^Example usage:|^Troubleshooting:|^Contributing:" # |^Further help:"
$usagePattern = "^Usage: (?<usage>.*)"
$optionPattern = "^      --\w|^  -\w"
$optionAdvance = $false
$argumentPattern = "NoMatchingArguments"
$subCommandIncludesExe = $true
$headerLength = 0
...
function parseHelp([string]$exe, [string[]]$commandProlog) {
    ...
    for($i = $offset; $i -lt $helpText.Count; $i++) {
        if ($helpText[$i] -match $usagePattern) {
            $usage = $matches['usage']
            $positionalParameters = CheckForPositionalParameters($helpText[$i])
        ...
        elseif ($helpText[$i] -match $optionPattern) {
            ...
            while($helpText[$i] -ne "" -and $i -lt $helpText.Count) {
                if ($helpText[$i] -match $parmPattern) {
                    ...
                    $p = [cParameter]::new($pName, $originalName, "$pHelp".Trim())
                    $p.OriginalText = $helpText[$i].Trim()
                    if ( $matches['ptype'] ) {
                        $pType = $matches['ptype']
...
function CheckForPositionalParameters([string]$helpText) {
    if ( $helpText -match "\[options\] (?<pParam>.*)") {
        if ( $matches['pParam'] -match "\.\.\." ) {
            $pType = "string[]"
        }
        else {
            $pType = "string"
        ...
        $isMandatory = $true
        if ( $helpText -match "\[options\] \[" ) {
            $isMandatory = $false
        }
        $pText = $matches['pParam'] -replace "\.\.\." -replace "\[" -replace "\["
        $pName, $extra = $pText.Split("|")
        # we have no real parameter name or help text.
        $p = [cParameter]::new($pName, "", "")
        $p.Position = 0
