...
class GitCommand {
    ...
    [object]GetCrescendoCommand() {
        ...
        if ( $this.Usage.Count -gt 0 ) {
            $c.Usage = New-UsageInfo -usage $this.Usage[0]
        }
...
function parseUsage([string[]]$helpText, [ref]$offset) {
    $usage = @()
    $i = $offset.value
    $strs = @()
    $strs += $helpText[$i]
    # read lines until the first blank line
    while(($i -lt $helpText.Count) -and ($helpText[$i+1] -ne "")) {
        $i++
        $text = $helpText[$i]
        if ($text -match "^   or:") {
            $usage += ($strs -join " ") -replace "\s\s+"," "
            $strs = @()
            $strs += "$text".Trim()
        }
        else {
...
function parseHelp([string]$exe, [string[]]$commandProlog) {
    ...
    [array]$helpText = @()
    if ( $commandProlog ) {
        $helpText = & $exe $commandProlog $helpChar 2>&1
    }
    else {
        $helpText = & $exe $helpChar 2>&1
    }
    $offset = $headerLength
    #$cmdhelp = @()
    #while ( $helpText[$offset].Length -ne 0) {
    ...
    for($i = $offset; $i -lt $helpText.Count; $i++) {
        if ($helpText[$i] -match $usagePattern) {
            $usage = parseUsage -helpText $helpText -offset ([ref]$i)
        }
